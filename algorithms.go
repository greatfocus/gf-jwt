...
func (a *Algorithm) Sign(unsignedToken string) ([]byte, error) {
	...
	if err != nil {
		return nil, Wrap(err, "Unable to write to HMAC-SHA256")
	}
...
func (a *Algorithm) Encode(payload *Claims) (string, error) {
	...
	if err != nil {
		return "", Wrap(err, "unable to marshal header")
	}
	...
	if err != nil {
		return "", Wrap(err, "unable to marshal payload")
	}
	...
	if err != nil {
		return "", Wrap(err, "unable to sign token")
	}
...
func (a *Algorithm) Decode(encoded string) (*Claims, error) {
	...
	if err != nil {
		return nil, Wrap(err, "unable to decode base64 payload")
	}

	if err := json.Unmarshal(payload, &claims); err != nil {
		return nil, Wrap(err, "unable to unmarshal payload json")
	}
...
func (a *Algorithm) DecodeAndValidate(encoded string) (claims *Claims, err error) {
	...
	if err = a.validateSignature(encoded); err != nil {
		err = Wrap(err, "failed to validate signature")
		return
	}

	if err = a.validateExp(claims); err != nil {
		err = Wrap(err, "failed to validate exp")
		return
	}

	if err = a.validateNbf(claims); err != nil {
		err = Wrap(err, "failed to validate nbf")
	}
...
func (a *Algorithm) validateSignature(encoded string) error {
	...
	if err != nil {
		return Wrap(err, "unable to sign token for validation")
	}
