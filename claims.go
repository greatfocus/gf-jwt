...
func (c Claims) Get(key string) (interface{}, error) {
	...
	if !ok {
		return "", Wrapf(nil, "claim (%s) doesn't exist", key)
	}
...
func (c *Claims) GetTime(key string) (time.Time, error) {
	raw, err := c.Get(key)
	if err != nil {
		return time.Unix(0, 0), Wrapf(err, "claim (%s) doesn't exist", key)
	}

	timeFloat, ok := raw.(float64)
	if !ok {
		return time.Unix(0, 0), Wrap(err, "claim isn't a valid float")
	}
